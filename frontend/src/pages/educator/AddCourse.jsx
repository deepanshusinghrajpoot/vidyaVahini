...
const AddCourse = () => {
  ...
  // Function to add chapter Or handle chapter
 ...
 // Function to add Lecture
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 p-4 pb-0'>
        <form onSubmit={handleSubmit}
              className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
            <div className='flex flex-col gap-1'>
                <p>Course Title</p>
                <input onChange={e=> setCourseTitle(e.target.value)} value={courseTitle}  type="text" required 
            ...
            <div className='flex flex-col gap-1'>
                  <p>Descount %</p>
                  <input onChange={e=> setDiscount(e.target.value)} value={discount}   type="number" placeholder='0' min={0} max={100} required
                         className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' />
            </div>
                    ...
                    showPopup && (
                      <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
                           <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                               ...
                               <div className='mb-2'>
                                   <p>Lecture Title</p>
                                   <input onChange={e => setLectureDetails({...lectureDetails, lectureTitle: e.target.value })} value={lectureDetails.lectureTitle} 
                                          type="text" className='mt-1 block w-full border rounded py-1 px-2' />
                               </div>
                               <div className='mb-2'>
                                   <p>Duration (minutes)</p>
                                   <input onChange={e => setLectureDetails({...lectureDetails, lectureDuration: e.target.value })} value={lectureDetails.lectureDuration} 
                                          type="number" className='mt-1 block w-full border rounded py-1 px-2' />
                               </div>
                               <div className='mb-2'>
                                   <p>Lecture URL</p>
                                   <input onChange={e => setLectureDetails({...lectureDetails, lectureUrl: e.target.value })} value={lectureDetails.lectureUrl} 
                                          type="text" className='mt-1 block w-full border rounded py-1 px-2' />
                               </div>
                               <div className='flex gap-2 my-4'>
                                   <p>Is Preview Free?</p>
                                   <input onChange={e => setLectureDetails({...lectureDetails, isPreviewFree: e.target.checked })} checked={lectureDetails.isPreviewFree} 
